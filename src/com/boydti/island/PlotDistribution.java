...
public class PlotDistribution implements IslandDistribution {
    ...
    public PlotDistribution(final String[] args) {
        ICLogger.logger.info("Creating PlotDistribution with args: " + StringUtils.join(args, " "));
        if (args.length != 2) {
            ICLogger.logger.severe("PlotDistribution requrires 2 parameters, " + args.length + " given");
        ...
        if (islandSize <= 0 || islandSize % 32 != 0) {
            ICLogger.logger.severe("PlotDistribution.island-size must be a positive multiple of 32");
        ...
        if (oceanSize <= 0 || oceanSize % 32 != 0) {
            ICLogger.logger.severe("PlotDistribution.ocean-size must be a positive multiple of 32");
    ...
    public ICLocation getCenterAt(int x, int z, final long worldSeed) {
        ...
        final int zPrime = z + innerRadius;
        ...
        final int xPrime = x + innerRadius;
